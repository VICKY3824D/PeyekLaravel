...
<link rel="stylesheet" href="{{ asset('assets/detail/style.css') }}">
@endsection
        ...
        <div class="success-card">
                    ...
                    <table class="table table-custom">
                        ...
                        <tbody>
                            ...
                            @foreach($order->orderItems as $item)
                                @php
                                    $subtotal = $item->qty * $item->hrg_kiloan;
                                    $subtotalItems += $subtotal;
                                @endphp
                                <tr>
                                    <td class="product-name">{{ $item->item->nama_peyek }}</td>
                                    <td>{{ $item->qty }} kg</td>
                                    <td>Rp{{ number_format($item->hrg_kiloan, 0, ',', '.') }}</td>
            ...
            <div class="action-buttons">
                @if($order->status === 'belum bayar')
                    <form action="{{ route('order.bayar', $order->id) }}" method="POST" style="margin: 0;">
                        @csrf
                ...
                @elseif($order->status === 'selesai')
                    <a href="{{ route('customer.order.review', $order->id) }}" class="btn btn-custom-success">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" style="vertical-align: middle; margin-right: 8px;" viewBox="0 0 16 16">
                ...
                <a href="{{ route('landing') }}" class="btn btn-custom-secondary">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" style="vertical-align: middle; margin-right: 8px;" viewBox="0 0 16 16">
